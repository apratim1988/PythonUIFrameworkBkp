...
class Test_RSA_FW(BaseTest):
    ...
    @pytest.mark.parametrize(
        "model,RTO,expdate,mobileno,fullname,email,regno,engno,chssno,finname,pucno,pucexpdt,prevpolno,nomname,nomage,pan,aadhar,add1,add2,pin",
        dataProvider.get_data("rsa_fourwheeler"))
    def test_RSA_FW(self, model, RTO, expdate, mobileno, fullname, email, regno, engno, chssno, finname, pucno,
...
#        self.VerifyPresence_RSAFWSelectBrandButton()
        ...
        time.sleep(1)
        rsafwselectvehiclepage.SelectVehicleModel1()
        time.sleep(1)
